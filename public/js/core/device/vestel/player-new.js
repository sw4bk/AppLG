...
/**
 * Media Player for for Vestel/GoGEN platforms (probably also Hyundai).
 * 
 * DRM agent support (currently not supported by vestel firmware)
 * 
 * @author AuroraTech
 * @class Device_Vestel_Player_New
 * @mixins Events
 * @extends Player
 */

Device_Vestel_Player_New = (function(Events) {
	var Device_Vestel_Player_New = {
		STATE_IDLE: -1,
	...
	$.extend(true, Device_Vestel_Player_New, Events, {
		/**
		...
		sendLicenceRequest:function(xml) {
			this.drmplugin.sendDRMMessage('application/vnd.ms-playready.initiator+xml', xmlLicenceAcquisition, 'urn:dvb:casystemid:19219');
		},

		/**
		 * Handle the DRMagent's DRMMessageResult event
		 * @param {Number} msgID
		 * @param {String} resultMsg
		 * @param {Number} resultCode
		 */
		HandleOnDRMMessageResult: function(msgID, resultMsg, resultCode) {
			//alert("HandleOnDRMMessageResult: function("+msgID+", "+resultMsg+", "+resultCode+")");
			if (resultCode == 0) {

				var url = Player.url;
				Player.setURL(url);
				//alert("Player.resumeTime = "+Player.resumeTime);
		...
		/**
		 * Handle the DRMagent's DRMRightsError event
		 */
	...
	return Device_Vestel_Player_New;
